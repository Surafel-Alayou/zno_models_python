...
stacking_X = pd.DataFrame({'cat_pred': cat_preds, 'xgb_pred': cat_preds, 'gb_pred': gb_preds})
...
plt.scatter(y_test, stacking_preds, color='red', label='Predictions')
plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], linestyle='--', color='blue', label='Ideal')
plt.xlabel('Actual (nm)')
plt.ylabel('Predicted (nm)')
r2_stack = r2_score(y_test, stacking_preds)
plt.title(f'Stacked: R\u00b2 = {r2_stack:.4f}')
plt.legend()
plt.show()
